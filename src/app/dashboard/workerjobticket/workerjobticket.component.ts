...
@Component({
  selector: 'app-workerjobticket',
  templateUrl: './workerjobticket.component.html',
  styleUrls: ['./workerjobticket.component.css']
})
export class WorkerjobticketComponent implements OnInit {
  jobTicketList = []
  modalRef: BsModalRef;
  ...
  ngOnInit() {
    ...
    this.getAllJobTicketByUser(this.id);
  }
  ...
  response = false;
  getAllJobTicketByUser(id) {
    this.jobservice.getJobByUser(id).subscribe(res => {
      this.user = res;
